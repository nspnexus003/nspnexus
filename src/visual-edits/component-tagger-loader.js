    ...
    try {
        ...
        (0, estree_walker_1.walk)(ast, {
            enter(node) {
                ...
                let nspNexusId = `${rel}:${line}:${column}`;
                // Enhance the ID with context if we have map information
                if (mapContext) {
                    nspNexusId += `@${mapContext.arrayName}`;
                }
                ...
                ms.appendLeft(node.name.end, ` data-nsp-nexus-id="${nspNexusId}" data-nsp-nexus-name="${semanticName}"`);
                mutated = true;
            },
        });
        if (!mutated)
